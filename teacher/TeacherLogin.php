...
<?php
	...
	if(isset($login))
	{
		$que=mysqli_query($conn,"select * from teacher where email='$txtEMail' and password='$txtPassword'");
		$row=mysqli_num_rows($que);
		if($row)
			{	
				$que=mysqli_query($conn,"select * from teacher where email='$txtEMail' and password='$txtPassword' and status='approved'");
				$row=mysqli_num_rows($que);
				if($row)
				{
					$row = mysqli_fetch_array($que);
					$_SESSION['teacher']=$txtEMail;
					$_SESSION['pic']=$row[6];
					$_SESSION['tname']=$row[1];
				
					header('location:dashboard/teacherdashboard.php');
				}
				else
				{
...
<body>
    ...
    <div class="inner-banner">
        <section class="w3l-breadcrumb">
            <div class="container">
                <h4 class="inner-text-title font-weight-bold text-white mb-sm-3 mb-2">Teacher Login</h4>
                <ul class="breadcrumbs-custom-path">
                    <li><a href="index.html">Home</a></li>
                    <li class="active"><span class="fa fa-chevron-right mx-2" aria-hidden="true"></span> Teacher Login</li>
                </ul>
            </div>
        </section>
    </div>
            ...
            <div class="mx-auto" style="max-width:1000px">
                ...
                <div class="contact-form-sec pt-5 mt-md-5">
					...
					<h2 class="title-main"><?php echo @$err;?></h3>
                        <h3 class="title-main">Login for <span>TEACHER</span></h3>
                    </div>
